...
import { Search, ExternalLink, Plus, X, FileText, GitBranch, AlertTriangle, MoreHorizontal, Upload } from 'lucide-react';
...
- Use case 3`
  ...
  const handleStatusChange = (newStatus) => {
    ...
    // Update plugin in the main plugins list
    if (selectedPlugin) {
      const updatedPlugins = plugins.map(p => {
        if (p.id === selectedPlugin.id) {
          return { 
            ...p, 
            visibility: newStatus === 'Public' ? 'public' : 'draft',
            status: newStatus === 'Public' ? 'Public' : 'Draft'
          };
        }
        return p;
      });
  ...
  const handleActionDropdownClick = (pluginId, event) => {
    event.stopPropagation();
    setActiveDropdown(activeDropdown === pluginId ? null : pluginId);
  };

  const handleViewInMarketplace = (plugin, event) => {
    event.stopPropagation();
    window.open(`/plugin/${plugin.id}`, '_blank');
    setActiveDropdown(null);
  };

  const handleUnpublish = (plugin, event) => {
    event.stopPropagation();
    
    // Update plugin status to draft
    const updatedPlugins = plugins.map(p => {
      if (p.id === plugin.id) {
        return { ...p, visibility: 'draft' };
      }
      return p;
    });
    ...
    console.log(`Plugin "${plugin.title}" has been unpublished and set to Draft status`);
  };

  const handlePublish = (plugin, event) => {
    event.stopPropagation();
    
    // Update plugin status to published
    const updatedPlugins = plugins.map(p => {
      if (p.id === plugin.id) {
    ...
    // Show confirmation message
    console.log(`Plugin "${plugin.title}" has been published and is now visible in the Marketplace`);
  };
                        ...
                        >
                          ...
                          <div className="plugin-action-dropdown" ref={activeDropdown === plugin.id ? dropdownRef : null}>
                            <button
                              ...
                              className="plugin-action-button-new"
                            ...
                            {activeDropdown === plugin.id && (
                              <div className="plugin-dropdown-menu">
                                <button
                                  onClick={(e) => handleViewInMarketplace(plugin, e)}
                                  className="plugin-dropdown-item"
                                >
                                  <ExternalLink size={16} />
                                  View in Marketplace
                                </button>
                                {plugin.visibility === 'draft' ? (
                                  <button
                                    onClick={(e) => handlePublish(plugin, e)}
                                    className="plugin-dropdown-item"
                                  >
                                    <Upload size={16} />
                                    Publish
                                  ...
                                  <button
                                    onClick={(e) => handleUnpublish(plugin, e)}
                                    className="plugin-dropdown-item"
                                  >
                                    <X size={16} />
                                    Unpublish
              ...
              paddingTop: '24px'
                ...
                <div style={{
                  ...
                  <div style={{ marginBottom: '32px' }}>
                    <p style={{ 
                      ...
                      <span>Plugins / </span>
                      <span>{selectedPlugin?.title || selectedPlugin?.name || '3D Building Visualization'}</span>
                    </p>
                  </div>
                ...
                <div 
                  ...
                  <div style={{ 
                    ...
                    <div 
                      ...
                      style={{ 
                        ...
                        display: activeSection === 'General' ? 'block' : 'none'
                      }}
                    >
                    ...
                    {/* Action Buttons - Show only in General section */}
                    <div style={{ 
                      display: 'flex', 
                      gap: '12px', 
                      paddingTop: '20px',
                  ...
                  <div 
                    ...
                    style={{ 
                      scrollMarginTop: '24px',
                      display: activeSection === 'README' ? 'block' : 'none'
                    }}
                  >
                  ...
                  <div 
                    ...
                    style={{ 
                      scrollMarginTop: '24px',
                      display: activeSection === 'Version' ? 'block' : 'none'
                    }}
                  >
                    <div style={{ marginBottom: '24px' }}>
                  ...
                  <div 
                    ...
                    style={{ 
                      scrollMarginTop: '24px',
                      display: activeSection === 'Danger Zone' ? 'block' : 'none'
                    }}
                  >
                    <div style={{ marginBottom: '24px' }}>
                      <h2 
